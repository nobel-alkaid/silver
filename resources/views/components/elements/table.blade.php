            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-center">
                        ...
                        <a class="flex items-center text-sm hover:opacity-80 justify-center ">
                            ...
                            <div class="relative h-10 mr-3 md:block">
                                <img class="object-cover w-full h-full rounded-full"
                                    src="{{ url('storage/' . $resource->{$column}) }}" alt="" loading="lazy">
                            </div>
                            {{-- <div>
                        ...
                        {{ $resource->{$column}->number ?? $resource->{$column}->name ?? $resource->{$column}->fullname
                        }}
                    ...
                    <td class="px-4 py-3 text-center">
                        <div class="flex items-center space-x-4 text-sm justify-center">
                            ...
                            <form
                                action="{{ route('dashboard.'. Str::plural($type) . '.destroy', [$type => $resource]) }}"
                                method="POST" onsubmit="return confirm('Voulez vous supprimer cet élément ?')">
                                @method('DELETE')
                                @csrf
                                <button type="submit"
